...
final class Schema implements Bootable {
	...
	private function organization(): array {
		return [
			...
			'url'         => home_url( '/' ),
			'logo'        => $this->logo_url(),
			'telephone'   => get_option( 'greenworld_phone', '+254708777192' ),
			'email'       => get_option( 'greenworld_email', 'info@greenworldhealthsolutions.co.ke' ),
			'address'     => [
				'@type'           => 'PostalAddress',
				'streetAddress'   => get_option( 'greenworld_street', 'Magomano House, 1st Floor, Room 10D, Tom Mboya Street' ),
	...
	private function product_schema(): ?array {
		...
		$gtin = (string) get_post_meta( $product->get_id(), '_greenworld_gtin', true );
		$mpn  = (string) get_post_meta( $product->get_id(), '_greenworld_mpn', true );
		if ( strlen( $mpn ) === 0 ) {
	...
	private function build_offers( \WC_Product $product ): array {
		...
		$gw_sd = $this->shipping_details();
		if ( count( $gw_sd ) > 0 ) {
			$offer['shippingDetails'] = $gw_sd;
		}
		return $offer;
	}
	...
	private function breadcrumbs(): array {
		...
		$items[]  = $this->crumb( $pos++, __( 'Shop', 'greenworld' ), (string) $shop_url );
